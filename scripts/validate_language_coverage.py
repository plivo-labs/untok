...
def main(argv=None):
    ...
    parser.add_argument("--policy", type=Path, default=Path("configs/language-coverage.json"))
    ...
    fetch.add_argument("--output", type=Path, default=Path(".cache/language-coverage/prepared-v1"))
    fetch.add_argument("--offline", action="store_true", help="Use already hash-verified TSV files")
    validate = sub.add_parser("validate", help="Evaluate all three installed bundles")
    validate.add_argument("--corpora", type=Path, default=Path(".cache/language-coverage/prepared-v1/manifest.json"))
